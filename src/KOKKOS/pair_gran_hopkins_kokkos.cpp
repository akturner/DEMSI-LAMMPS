...
// history[5] : delta_0: initial overlap at bond break
// history[6] : flag for whether plastic deformation has occurred
// history[7] : previousForce
// history[8] : chi1
// history[9] : chi2
// history[10] : ridgeSlip
// history[11] : ridgeSlipUsed

//For unbonded pairs (new):
// history[0,1]: chi1, chi2
// history[2,3]: accumulated tangential displacement at contact, x and y
...
// history[8]  : previousForceShear
// history[9]  : ridgeSlipShear
// history[10] : ridgeSlipUsedShear
                     ...
                     F_FLOAT &fx, F_FLOAT &fy, F_FLOAT &torque_i, F_FLOAT &torque_j) const
{
   ...
   if (rsq >= radsum*radsum){
     ...
     // elements in contact
     if (!d_firsttouch(i,jj)){
          //If this is first contact
          d_firsttouch(i,jj) = 1;
     ...
     if (type(i) == 2 || type(j) == 2){
       ...
       F_FLOAT previousForceConv = d_firsthistory(i,size_history*jj+5);
       F_FLOAT ridgeSlipConv     = d_firsthistory(i,size_history*jj+6);
       F_FLOAT ridgeSlipUsedConv = d_firsthistory(i,size_history*jj+7);
       ...
       if (HISTORYUPDATE){
         d_firsthistory(i,size_history*jj+5) = previousForceConv;
         d_firsthistory(i,size_history*jj+6) = ridgeSlipConv;
         d_firsthistory(i,size_history*jj+7) = ridgeSlipUsedConv;
                     ...
                     F_FLOAT &fx, F_FLOAT &fy, F_FLOAT &torque_i, F_FLOAT &torque_j) const
{
  ...
  if (NEWTON_PAIR || j < nlocal){
    ...
    rxj = d_firsthistory(i,size_history*jj+2) + 0.5*s1x - x(j,0);
    ryj = d_firsthistory(i,size_history*jj+3) + 0.5*s1y - x(j,1);
    Bnj = rxj*bey - ryj*bex;
    Cnj = -Cn;
    Dnj = -Dn;
    Btj = rxj*mey - ryj*mex;
  ...
  if (HISTORYUPDATE){
    ...
    d_firsttouch(i,jj) = 1;
    if (d_firsthistory(i,size_history*jj) >= d_firsthistory(i,size_history*jj+1)){ //Bond just broke
